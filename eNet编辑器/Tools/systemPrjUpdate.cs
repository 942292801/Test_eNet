...
{
    public partial class systemPrjUpdate : Form
    {
        public systemPrjUpdate()
        {
            InitializeComponent();
        }
        ...
        private void systemPrjUpdate_Load(object sender, EventArgs e)
        {
            //IPIni(); 暂时废除这个从项目获取的
            udpreceviceDelegate += new Action<string>(udpReceviceDelegateMsg);
            findOnlineGW();
            findIniFilePath(txtPath, "systemPrjPath");
        ...
        private void IPIni()
        {
            ...
            {
                cbOnlineIP.Items.Add(d.ip);
            }
            if (cbOnlineIP.Items.Count > 0)
            {
                cbOnlineIP.SelectedIndex = 0;
            }
        }
        ...
        #endregion
        private void systemPrjUpdate_Paint(object sender, PaintEventArgs e)
        {
        ...
        {
            openfile(txtPath, "systemPrjPath", "prj");
        }
        ...
        public bool openfile(TextBox tb, string key, string type)
        {
            try
            {
                ...
                if (type == "json")
                {
                    op.Filter = "文件（*.json）|*.json|工程文件（*.zip）|*.zip|固件文件（*.prj）|*.prj|All files(*.*)|*.*";
                }
                else if (type == "prj")
                {
                    op.Filter = "固件文件（*.prj）|*.prj|工程文件（*.zip）|*.zip|文件（*.json）|*.json|All files(*.*)|*.*";
                }
                else
                {
                    op.Filter = "工程文件（*.zip）|*.zip|固件文件（*.prj）|*.prj|文件（*.json）|*.json|All files(*.*)|*.*";
                }
